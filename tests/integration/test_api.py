...
class TestPredictEndpoint:
    ...
    def test_predict_invalid_json_returns_422(self, test_client):
        ...
        response = test_client.post("/predict", content="invalid json", headers={"Content-Type": "application/json"})
        assert response.status_code == 422
    ...
    def test_predict_multiple_valid_requests(self, test_client, known_user_movie_pairs):
        ...
        for pair in known_user_movie_pairs:
            req = {"user_id": pair["user_id"], "movie_id": pair["movie_id"]}
...
class TestErrorHandling:
    ...
    def test_unloaded_model_returns_503(self, test_client, sample_prediction_request, sample_batch_request):
        ...
        try:
            ...
            r1 = test_client.post("/predict", json=sample_prediction_request)
            assert r1.status_code == 503
            r2 = test_client.post("/predict/batch", json=sample_batch_request)
            assert r2.status_code == 503
        finally:
            main_module.model = saved_model

    def test_prediction_error_returns_500(self, test_client, sample_prediction_request, sample_batch_request):
        ...
        saved_model = main_module.model
        try:
            main_module.model = FailingModel()
            r1 = test_client.post("/predict", json=sample_prediction_request)
