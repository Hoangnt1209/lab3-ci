...
class TestHealthResponse:
    ...
    def test_health_response_status_types(self):
        ...
        r1 = HealthResponse(status="healthy", model_loaded=True)
        r2 = HealthResponse(status="unhealthy", model_loaded=False)
        assert r1.status == "healthy" and r1.model_loaded is True
        assert r2.status == "unhealthy" and r2.model_loaded is False
...
class TestBatchPredictionRequest:
    ...
    def test_valid_batch_request(self):
        ...
        request = BatchPredictionRequest(predictions=items)
